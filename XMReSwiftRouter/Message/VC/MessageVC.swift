...
class MessageVC: UIViewController, UITableViewDelegate, Routable {
    ...
    func setupUI() {
        ...
        let loadItem = UIBarButtonItem(title: "reload", style: .plain, target: self, action: #selector(self.reloadItem(sender:)))
        self.navigationItem.rightBarButtonItems = [loadItem, addItem, changeItem, delItem]
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Home", style: .plain, target: self, action: #selector(self.navToHome(sender:)))
    }
...
//        appStore.unsubscribe(self)
    ...
    @objc func navToHome(sender: AnyObject) {
        appStore.dispatch(SetRouteAction([RouteNames.home]))
    }
